...
- **多用户隔离**：每个用户拥有独立的订阅、分类、标签、付款、通知和个人设置
- **用户管理**：管理员可在系统设置中添加、删除用户和重置密码
- **安全认证**：bcrypt 密码哈希 + JWT + CSRF 保护
- **全中文界面**
- **Docker 一键部署**
...
管理员可以在“系统设置 → 用户管理”中创建普通用户或其他管理员、重置用户密码和删除用户。
删除用户会同时删除该用户的订阅及相关个人数据，系统禁止删除当前登录账户。
